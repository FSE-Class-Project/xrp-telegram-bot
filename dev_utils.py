...
async def test_xrp_connection():
    ...
    try:
        ...
        if response.is_successful():
            print("✅ Connected to XRP Ledger TestNet")
            print(f"   Network: {xrp_service.network}")
            print(f"   URL: {xrp_service.json_rpc_url}")
            return True
        else:
            print("❌ Failed to connect to XRP Ledger")
            return False
...
async def create_test_wallet():
    ...
    try:
        ...
        if funded:
            print("   ✅ Wallet funded with TestNet XRP")
            ...
            print("   ⚠️ Failed to fund wallet (faucet may be rate-limited)")
...
async def test_api():
    ...
    try:
        ...
        print("   Start it with: python run.py backend")
        return False
    except Exception as e:
        print(f"❌ API error: {e}")
        return False
