...
from backend.api.middleware import limiter, get_idempotency_key, rate_limit_transactions
...
def validate_transaction_feasibility(
    ...
    if current_balance < XRPConstants.MIN_ACCOUNT_BALANCE:
        return False, f"Account not activated. Minimum balance required: {XRPConstants.MIN_ACCOUNT_BALANCE} XRP"
    ...
    if available_balance < total_cost:
        return False, f"Insufficient balance. Available: {available_balance} XRP, Required: {total_cost} XRP (including fee)"
    ...
    if remaining_balance < XRPConstants.ACCOUNT_RESERVE:
        return False, f"Transaction would violate account reserve requirement. Minimum {XRPConstants.ACCOUNT_RESERVE} XRP must remain"
...
class SendTransactionRequest(BaseModel):
    ...
    def validate_amount(cls, v: Decimal) -> Decimal:
        ...
        try:
            ...
            if quantized != v:
                raise ValueError(f'XRP supports maximum {XRPConstants.MAX_DECIMAL_PLACES} decimal places')
        except Exception:
            raise ValueError('Invalid decimal precision for XRP amount')
        
        # Practical maximum for single transaction (anti-whale protection)
        if v > XRPConstants.PRACTICAL_MAX_TRANSACTION:
            raise ValueError(f'Single transaction amount cannot exceed {XRPConstants.PRACTICAL_MAX_TRANSACTION} XRP')
        
        # Check for dust amounts (very small transactions that waste network resources)
        if v < XRPConstants.DUST_THRESHOLD:
            raise ValueError(f'Amount too small: minimum practical amount is {XRPConstants.DUST_THRESHOLD} XRP')
...
class TransactionValidationRequest(BaseModel):
    ...
    def validate_amount(cls, v: Decimal) -> Decimal:
        ...
        return SendTransactionRequest.validate_amount(v)
...
async def send_transaction(
    ...
    from ..utils.idempotency import TransactionIdempotency, IdempotencyKey
    ...
    if transaction.amount > XRPConstants.LARGE_TRANSACTION_THRESHOLD:
        logger.warning(f"Large transaction attempted: {transaction.amount} XRP by user {sender.telegram_id}")
    ...
    if existing:
        ...
        if hasattr(existing, 'tx_hash'):  # It's a Transaction
            return TransactionResponse(
                ...
                recipient=existing.recipient_address,
                status=existing.status,
                message=f"Transaction already processed (idempotent)"
            )
    ...
    if result["success"] and "transaction_id" in result:
        ...
        tx_record = db.query(Transaction).filter(
            Transaction.id == result["transaction_id"]
        ).first()
        if tx_record:
            await tx_idempotency.link_transaction_to_idempotency(
                idempotency_record, tx_record
            )
    ...
    return TransactionResponse(
        ...
        message=result.get("message", "Transaction sent successfully")
    )
