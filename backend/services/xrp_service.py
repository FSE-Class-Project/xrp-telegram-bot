...
class XRPService:
    ...
    async def send_xrp(
        ...
        try:
            ...
            # Autofill required fields (Fee, Sequence, LastLedgerSequence)
            prepared = await autofill(payment, self.client)

            # Add buffer to LastLedgerSequence
            if prepared.last_ledger_sequence:
                prepared = replace(prepared, last_ledger_sequence=prepared.last_ledger_sequence + 50)
